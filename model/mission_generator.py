...
def mission_generator_daily_langchain(sub_categories: list[str]):
    ...
    sub_category = sub_categories[selected_idx]
    main_category = category_mapping[sub_category]
    prompt_description = prompt_descriptions[selected_idx]
    example_text = example_texts[selected_idx]
    ...
    chat_template = ChatPromptTemplate.from_messages([
        ...
        """),
        ("human", """
        질문: {main_category} 안에 {sub_category}를 바탕으로 개발자를 위한 {prompt_description} 1개를 생성하시오.
        답:
        """)
    ])
...
def mission_generator_free_langchain(sub_category: str):
    ...
    if main_category == "CS":
        if sub_category == "NETWORK":
            ...
            example_text = """
                    ...
                    """
        elif sub_category == "OPERATING_SYSTEM":
            example_text = """
    ...
    chat_template = ChatPromptTemplate.from_messages(
        ...
        사용자가 관심 있는 개발 분야는 {main_category}의 {sub_category}입니다.

        [행동 지침]
        1. 미션 제목만 출력합니다. 미션 설명, 미션 목표 등 추가 내용은 포함하지 않습니다.
        2. 미션은 반드시 {main_category}, {sub_category}와 관련된 주제여야 하며, 그 외 내용은 포함하지 않습니다.
        ...
        다음과 같은 JSON schema로 출력하세요:
        {{
            "mission_1": "높은 난이도 미션 제목(100자 이내)",
            "mission_2": "중간 난이도 미션 제목(100자 이내)",
            "mission_3": "쉬운 난이도 미션 제목(100자 이내)"
        }}
        """),
            ("human", """
    ...
    llm = LLMManager.get_instance().get_client()
    missions = json.loads(llm.invoke(messages).content)
